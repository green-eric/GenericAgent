...
### 发现3：大量文件归档至 archive/
```
fusion_picker.py      → archive/ (392行已删除)
sweep_v2.py          → archive/ (param_sweep结果)
full_backfill.py     → archive/
scheduled_backtest.py→ archive/
...
### P0（必须，不执行）
1. **恢复 daily_health_check.py** → 确认 run_daily_score.py 是否替代，或从archive恢复并部署定时任务
