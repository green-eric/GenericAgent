...
> 由 sop_pitfall_collector.py 维护，sop_pitfalls.json 存储

1. **凭记忆执行**：禁止！即使"记得上次怎么做的"也必须先读SOP的🔑速查段
2. **跳过验证步骤**：plan_sop第四章验证检查点不可跳过，即使"看起来没问题"
3. **主agent自己探测**：环境探测必须委托subagent，主agent上下文最宝贵
4. **探索subagent启动失败回退**：排查原因→重试(最多2次)，禁止主agent回退为自己探测
5. **步骤粒度过粗**：禁止"处理所有文件"这种步骤，必须展开到具体文件/操作
6. **漏标[D]**：读大量代码/网页/重复操作(>3次)的步骤必须标[D]委托subagent
7. **条件分支无降级方案**：高风险步骤(⚠标注)必须有明确的失败处理路径
