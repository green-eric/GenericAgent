...
def _init_cdp_driver():
    """初始化 CDP 驱动（离屏隐身 Chrome via --remote-debugging-port=9222）"""
    global _cdp_driver
    import socket
    try:
        s = socket.socket(); s.settimeout(2); s.connect(('127.0.0.1', 9222)); s.close()
    except:
        print("[CDP] 9222 端口不可用，尝试启动 Chrome...")
        import subprocess, os
        chrome_exe = r"C:\Program Files\Google\Chrome\Application\chrome.exe"
        if not os.path.exists(chrome_exe):
            print("[CDP] Chrome 不存在"); return None
        # 离屏隐身启动（--window-position=-32000,-32000）
        subprocess.Popen([
            chrome_exe,
        ...
        for _ in range(10):
            ...
            try:
                s = socket.socket(); s.settimeout(2); s.connect(('127.0.0.1', 9222)); s.close()
                print("[CDP] Chrome CDP 已就绪"); break
            except: pass
        else:
            print("[CDP] Chrome CDP 启动超时"); return None
    try:
        from cdp_bridge import CDPDriver
        _cdp_driver = CDPDriver(port=9222)
        print("[CDP] CDPDriver 初始化成功")
        return _cdp_driver
    except Exception as e:
        print(f"[CDP] CDPDriver 初始化失败: {e}"); return None

def first_init_driver():
    global driver, _cdp_driver
    # 先尝试 TMWebDriver（扩展模式）
    try:
        ...
        for i in range(5):
            ...
            if len(sess) > 0:
                print(f"[Driver] TMWebDriver 就绪，{len(sess)} 个 session")
                return
        print("[Driver] TMWebDriver 无 session，回退到 CDP")
    except Exception as e:
        print(f"[Driver] TMWebDriver 初始化失败: {e}")
        driver = None
    # 回退到 CDP
    cdp = _init_cdp_driver()
    if cdp:
        driver = cdp
        print("[Driver] 已切换到 CDP 模式（离屏隐身 Chrome）")
    else:
        print("[Driver] TMWebDriver 和 CDP 都不可用！")
