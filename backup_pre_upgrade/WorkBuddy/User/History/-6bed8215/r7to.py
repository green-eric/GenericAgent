...
def main():
    ...
    def hdr(ws, row, cols):
        for c, h in enumerate(cols, 1):
            ...
            cell.font = HF; cell.fill = HFL; cell.alignment = CT; cell.border = BD
    def sty(ws, r1, r2, nc):
        for r in range(r1, r2+1):
            for c in range(1, nc+1):
                ...
                if not cell.font or cell.font.name != '微软雅黑':
                    cell.font = NF
                cell.alignment = CT if c <= 3 else LT
                cell.border = BD
    def wid(ws, nc, mx=40):
        for c in range(1, nc+1):
            ml = 8
            for row in ws.iter_rows(min_col=c, max_col=c, values_only=True):
                for v in row:
                    if v: ml = max(ml, min(len(str(v))*1.2, mx))
    ...
    ws1.cell(row=1, column=1, value=f"2025年报归母净利润增长率 > 50%（共{len(high_2025)}只，排除科创板）").font = TF
    ws1.cell(row=1, column=1).alignment = CT
    r = 3; hdr(ws1, r, ["排名","股票名称","代码","2025年报净利(亿)","2024年报净利(亿)","增长率(%)","报告期","数据源"])
    for i, s in enumerate(high_2025, 1):
        ...
        ws1.cell(row=r, column=4, value=s['profit_2025ann'])
        ws1.cell(row=r, column=5, value=s['profit_2024ann'])
        ws1.cell(row=r, column=6, value=s['growth_2025ann'])
        ws1.cell(row=r, column=7, value=s['status'])
        ws1.cell(row=r, column=8, value="NeoData")
        ws1.cell(row=r, column=6).fill = RF
    sty(ws1, 4, r, 8); wid(ws1, 8)
    
    # Sheet2: 2026一季报增长>50%
    ws2 = wb.create_sheet("2026一季报增长>50%")
    ws2.merge_cells('A1:H1')
    ws2.cell(row=1, column=1, value=f"2026一季报归母净利润增长率 > 50%（共{len(high_2026q1)}只，排除科创板）").font = TF
    ws2.cell(row=1, column=1).alignment = CT
    r = 3; hdr(ws2, r, ["排名","股票名称","代码","2026Q1净利(亿)","2025Q1净利(亿)","增长率(%)","报告期","数据源"])
    for i, s in enumerate(high_2026q1, 1):
        ...
        ws2.cell(row=r, column=4, value=s['profit_2026q1'])
        ws2.cell(row=r, column=5, value=s['profit_2025q1'])
        ws2.cell(row=r, column=6, value=s['growth_2026q1'])
        ws2.cell(row=r, column=7, value=s['status'])
        ws2.cell(row=r, column=8, value="NeoData")
        ws2.cell(row=r, column=6).fill = RF
    sty(ws2, 4, r, 8); wid(ws2, 8)
    
    # Sheet3: 合并
    he = [r for r in results if (r['growth_2025ann'] is not None and r['growth_2025ann'] > 50) or (r['growth_2026q1'] is not None and r['growth_2026q1'] > 50)]
    he.sort(key=lambda x: max(x['growth_2025ann'] or 0, x['growth_2026q1'] or 0), reverse=True)
    ws3 = wb.create_sheet("合并(任一>50%)")
    ws3.merge_cells('A1:I1')
    ws3.cell(row=1, column=1, value=f"2025年报或2026一季报增长率 > 50%（共{len(he)}只）").font = TF
    ws3.cell(row=1, column=1).alignment = CT
    r = 3; hdr(ws3, r, ["排名","股票名称","代码","2025年报增长(%)","2026Q1增长(%)","2025年报净利(亿)","2024年报净利(亿)","2026Q1净利(亿)","2025Q1净利(亿)"])
    for i, s in enumerate(he, 1):
        ...
        ws3.cell(row=r, column=6, value=s['profit_2025ann'])
        ws3.cell(row=r, column=7, value=s['profit_2024ann'])
        ws3.cell(row=r, column=8, value=s['profit_2026q1'])
        ws3.cell(row=r, column=9, value=s['profit_2025q1'])
        if s['growth_2025ann'] and s['growth_2025ann'] > 50: ws3.cell(row=r, column=4).fill = RF
        if s['growth_2026q1'] and s['growth_2026q1'] > 50: ws3.cell(row=r, column=5).fill = RF
    sty(ws3, 4, r, 9); wid(ws3, 9)
    
    # Sheet4: 全量
    ws4 = wb.create_sheet("全部股票明细")
    ws4.merge_cells('A1:I1')
    ws4.cell(row=1, column=1, value=f"全部{total}只股票增长率明细（排除科创板）").font = TF
    ws4.cell(row=1, column=1).alignment = CT
    r = 3; hdr(ws4, r, ["股票名称","代码","2025年报增长(%)","2026Q1增长(%)","2025年报净利(亿)","2024年报净利(亿)","2026Q1净利(亿)","2025Q1净利(亿)","备注"])
    for s in results:
        r += 1
        ...
        ws4.cell(row=r, column=5, value=s['profit_2025ann'])
        ws4.cell(row=r, column=6, value=s['profit_2024ann'])
        ws4.cell(row=r, column=7, value=s['profit_2026q1'])
        ws4.cell(row=r, column=8, value=s['profit_2025q1'])
        ws4.cell(row=r, column=9, value=s['status'])
        g25 = s['growth_2025ann']; g26 = s['growth_2026q1']
        if g25 is not None and g25 > 50: ws4.cell(row=r, column=3).fill = RF
        elif g25 is not None and g25 < 0: ws4.cell(row=r, column=3).fill = GF
        if g26 is not None and g26 > 50: ws4.cell(row=r, column=4).fill = RF
        elif g26 is not None and g26 < 0: ws4.cell(row=r, column=4).fill = GF
        if g25 is None and g26 is None:
            for c in range(1, 10): ws4.cell(row=r, column=c).font = Font(name='微软雅黑', size=10, color='999999')
    sty(ws4, 4, r, 9); wid(ws4, 9)
    
    outpath2 = os.path.join(desktop, "股票增长率分析_2025年报_2026一季报_排除科创板.xlsx")
    wb.save(outpath2)
    print(f"Excel saved: {outpath2}")
