...
def check_fields():
    ...
    for field_name, count in field_stats.items():
        ...
        status = "PASS" if percentage >= 90 else "WARN" if percentage >= 70 else "FAIL"
        complete = "YES" if count == total_stocks else "NO"
        if count < total_stocks:
            all_complete = False
        print(f"{field_name:20}: {count}/{total_stocks} ({percentage:.1f}%) {status} Complete:{complete}")
    ...
    for ts_code, net_profit, net_assets, roe in sample_rows:
        if net_assets and net_assets > 0:
            ...
            match = "OK" if abs(calculated_roe - roe) < 0.01 else "ERR"
    ...
    cursor = sqlite3.connect(db_path).cursor()
    annual_fallback = cursor.execute("SELECT COUNT(*) FROM quarterly_ttm_cache WHERE roe_ttm IS NULL").fetchone()[0]
    ...
    all_fields_pass = all(count == total_stocks for count in field_stats.values())
    roe_calc_pass = ttm_roe_success == ttm_roe_total

    print(f"所有字段完整: {'YES' if all_fields_pass else 'NO'}")
    print(f"ROE计算完整: {'YES' if roe_calc_pass else 'NO'}")
    print(f"总体状态: {'PASS' if all_fields_pass and roe_calc_pass else 'WARN'}")
