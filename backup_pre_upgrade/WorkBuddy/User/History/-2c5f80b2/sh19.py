...
# 第一轮：查2024年报净利润
print(f"=== 第1轮：查询2024年报净利润 ({total}只) ===")
net2024_map = {}
...
# 第二轮：查2023年报净利润
print(f"\n=== 第2轮：查询2023年报净利润 ({total}只) ===")
net2023_map = {}
for i, (name, ts_code) in enumerate(stocks):
    v = query_neodata(name, ts_code, "2023")
    net2023_map[ts_code] = v
    status = f"{v/1e8:.2f}亿" if v else "N/A"
    print(f"[{i+1:3d}/{total}] {name}({ts_code}): 2023={status}")
    time.sleep(0.25)

# 计算增长率
print(f"\n{'='*70}")
print(f"2024年报净利润增长率 > 50% 的股票：")
print(f"{'='*70}")
high_growth = []
for name, ts_code in stocks:
    n24 = net2024_map.get(ts_code)
    n23 = net2023_map.get(ts_code)
    if n24 is not None and n23 is not None and n23 != 0:
        yoy = (n24 - n23) / abs(n23) * 100
        results.append({"name": name, "ts_code": ts_code, "net2024": n24, "net2023": n23, "yoy": round(yoy, 2)})
        if yoy > 50:
            high_growth.append({"name": name, "ts_code": ts_code, "net2024": n24, "net2023": n23, "yoy": round(yoy, 2)})
    else:
        results.append({"name": name, "ts_code": ts_code, "net2024": n24, "net2023": n23, "yoy": None})
...
for r in high_growth:
    ...
    print(f"{r['name']:<10} {r['ts_code']:<13} {v24:>14.2f} {v23:>14.2f} {r['yoy']:>7.1f}%")

# 也打印所有能算出增长率的
print(f"\n--- 全部增长率一览 ---")
valid = [r for r in results if r["yoy"] is not None]
valid.sort(key=lambda x: x["yoy"], reverse=True)
print(f"{'名称':<10} {'代码':<13} {'增长率':>8}")
print("-" * 40)
for r in valid:
    flag = " ***" if r["yoy"] > 50 else ""
    print(f"{r['name']:<10} {r['ts_code']:<13} {r['yoy']:>7.1f}%{flag}")

# 保存
with open("growth_result.json", "w", encoding="utf-8") as f:
    json.dump({"high_growth_over50": high_growth, "all": results}, f, ensure_ascii=False, indent=2)
