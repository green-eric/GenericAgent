...
def fetch_quarterly(ts_code, name, token):
    ...
    if token == "dummy_token":
        ...
        if ts_code in mock_data:
            date, metrics = mock_data[ts_code]
...
def percentile_rank(value, values, reverse=False):
    ...
    reverse=False: 越大越好（如ROE），最大值=100，最小值=0
    reverse=True: 越小越好（如负债率），最小值=100，最大值=0
    算法：找 value 在排序后列表中的位置索引 i，得分 = i/(n-1)*100
    """
    if not values: return 50.0
    n = len(values)
    if n == 1: return 50.0
    if reverse:
        # 越小越好：降序排列，value 越小说明越靠前（在降序中位置越靠后）
        sorted_vals = sorted(values, reverse=True)
    else:
        # 越大越好：升序排列，value 越大说明越靠后
        sorted_vals = sorted(values)
    # 找 value 在排序列表中等于或刚好超过的位置
    for i in range(n):
        if reverse:
            # 降序中，value <= sorted_vals[i] 说明 value 应该排在第 i 位或更后
            if value <= sorted_vals[i]:
                # 但需要确认这是 value 的正确位置
                # 在降序中找第一个 <= value 的位置
                pass
        else:
            if value <= sorted_vals[i]:
                return (i / (n - 1)) * 100.0
    # 上面的循环对升序有效：找第一个 >= value 的位置
    # 但对降序需要不同的逻辑
    # 重新用更清晰的方式
    return 0.0  # placeholder, will be replaced
...
def _write_excel(path, quarterly_scored, annual_scored, preferred):
    ...
    pref_fill = PatternFill("solid", fgColor="DDEBF7")  # 交集优选淡蓝底色
    ...
    for row_idx, r in enumerate(quarterly_scored, 2):
        ...
        a_total = r.get("total_score")
        a_grade = r.get("grade")

        row_data = [
            r.get("ts_code", ""), r.get("name", ""), r.get("industry_l1", ""),
            a_total, a_grade,
            ...
            "★" if r["ts_code"] in pref_codes else "",
        ]
        for col_idx, val in enumerate(row_data, 1):
            fill = None
            if col_idx == 5 and a_grade in grade_fills:   # 年报等级
                fill = grade_fills[a_grade]
            elif col_idx == 11 and q_grade in grade_fills:  # 季报等级
                fill = grade_fills[q_grade]
            elif col_idx == 25 and val == "★":               # 交集优选
                fill = pref_fill
            align = left if col_idx in (1, 2, 3, 21) else center
            set_cell(ws1, row_idx, col_idx, val, align=align, fill=fill)

    ws1.freeze_panes = "A2"
    ws1.auto_filter.ref = f"A1:Y{len(quarterly_scored) + 1}"
...
def main():
    ...
    parser.add_argument("--local-test", action="store_true", help="本地测试模式（不调用API）")
    ...
    # 输出 Excel 报告
    excel_path = os.path.join(Config.OUTPUT_DIR, f"季度评分_{timestamp}.xlsx")
