...
class Config:
    ...
    OUTPUT_DIR = BASE_DIR
    DB_FILE = os.path.join("..", "年报", "stock_cache.db")
...
def fetch_quarterly(ts_code, name, token):
    if token == "dummy_token":
        mock = {
            "000001.SZ": ("20250930", {"gross_margin": 42.5, "net_margin": 18.2, "revenue_yoy": 15.6, "profit_yoy": 22.3}),
            "000002.SZ": ("20250930", {"gross_margin": 38.9, "net_margin": 16.8, "revenue_yoy": 12.4, "profit_yoy": 18.7}),
            "600000.SH": ("20250930", {"gross_margin": 55.2, "net_margin": 28.1, "revenue_yoy": 8.9, "profit_yoy": 12.5}),
            "600036.SH": ("20250930", {"gross_margin": 52.8, "net_margin": 26.7, "revenue_yoy": 9.2, "profit_yoy": 13.1}),
            "000858.SZ": ("20250930", {"gross_margin": 75.6, "net_margin": 32.4, "revenue_yoy": 18.3, "profit_yoy": 25.8}),
        }
        if ts_code in mock:
            date, metrics = mock[ts_code]
            return {"metrics": metrics, "report_date": date, "fetch_success": True}
        else:
...
def percentile_rank(value, values, reverse=False):
    """计算 value 在 values 中的百分位排名 (0-100)
    reverse=False: 越大越好，最大值为100
    reverse=True:  越小越好，最小值为100
    若所有值相等，返回 50
    使用位置索引算法：升序找第一个 v >= value 的位置，降序找第一个 v <= value 的位置
    """
    if not values:
        return 50.0
    ...
    # 位置索引算法：找到value在排序数组中的正确位置
    if reverse:
        # 降序排列（越小越好），找第一个 <= value 的位置
        sorted_vals = sorted(values, reverse=True)
        for i, v in enumerate(sorted_vals):
            if v <= value:
                return (i / (n - 1)) * 100.0
    else:
        # 升序排列（越大越好），找第一个 >= value 的位置
        sorted_vals = sorted(values)
        for i, v in enumerate(sorted_vals):
            if v >= value:
                return (i / (n - 1)) * 100.0
    
    # 如果没有找到匹配项，说明value是最大值（reverse=True）或最小值（reverse=False）
    return 100.0 if reverse else 0.0

def calc_quarterly_score(annual_data, quarterly_metrics, industry_groups, all_stocks, quarterly_report_date=""):
    ...
    if level == "low":
        total *= Config.LOW_COMP_PENALTY
    elif level == "ultra_low":
        total *= Config.LOW_COMP_PENALTY * Config.ULTRA_LOW_COMP_PENALTY
    ...
    return {
        ...
        "quarterly_report_date": quarterly_report_date,
...
# ========== Excel 输出 ==========
def _write_excel(path, quarterly_scored, annual_scored, preferred):
    if not HAS_OPENPYXL:
        logger.warning("未安装 openpyxl，跳过 Excel 输出（pip install openpyxl）")
        return
    ...
    pref_fill = PatternFill("solid", fgColor="DDEBF7")

    def set_hdr(ws, row, col, val, width=None):
        c = ws.cell(row=row, column=col, value=val)
        c.font = hdr_font; c.fill = hdr_fill; c.alignment = center; c.border = border
        if width: ws.column_dimensions[get_column_letter(col)].width = width

    def set_cell(ws, row, col, val, align=center, fill=None, bold=False):
        c = ws.cell(row=row, column=col, value=val)
    ...
    # 年报成绩映射
    annual_map = {a["ts_code"]: a for a in annual_scored}

    # ---------- 表1：全部股票评分汇总 ----------
    ws1 = wb.active; ws1.title = "全部评分"
    headers1 = [
        ("代码", 12), ("名称", 12), ("行业", 14),
        ("年报总分", 10), ("年报等级", 8), ("年报盈利", 10), ("年报成长", 10), ("年报现金流", 10), ("年报负债", 10),
        ("季报总分", 10), ("季报等级", 8), ("季报盈利", 10), ("季报成长", 10), ("季报现金流", 10), ("季报负债", 10),
        ("季报毛利率", 10), ("季报净利率", 10), ("季报营收同比", 10), ("季报利润同比", 10),
        ("数据来源", 14), ("置信度", 8), ("完整度", 8), ("季报日期", 12),
        ("交集优选", 8),
    ]
    for i, (h, w) in enumerate(headers1, 1):
        set_hdr(ws1, 1, i, h, w)
    ws1.row_dimensions[1].height = 22

    pref_codes = {r["ts_code"] for r in preferred}
    for row_idx, r in enumerate(quarterly_scored, 2):
        ts_code = r["ts_code"]
        ann = annual_map.get(ts_code, {})
        a_total = ann.get("total_score")
        a_grade = ann.get("grade")
        q_total = r.get("quarterly_total_score")
        q_grade = r.get("quarterly_grade")

        row_data = [
            ts_code, r.get("name", ""), r.get("industry_l1", ""),
            a_total, a_grade,
            ann.get("profit_score"), ann.get("growth_score"), ann.get("ocf_score"), ann.get("debt_score"),
            q_total, q_grade,
            r.get("profit_score"), r.get("growth_score"), r.get("ocf_score"), r.get("debt_score"),
            r.get("q_gross_margin"), r.get("q_net_margin"), r.get("q_revenue_yoy"), r.get("q_profit_yoy"),
            r.get("data_source", ""), r.get("confidence", ""), r.get("completeness", ""),
            r.get("quarterly_report_date", ""),
            "★" if ts_code in pref_codes else "",
        ]
        for col_idx, val in enumerate(row_data, 1):
            fill = None
            if col_idx == 5 and a_grade in grade_fills:
                fill = grade_fills[a_grade]
            elif col_idx == 11 and q_grade in grade_fills:
                fill = grade_fills[q_grade]
            elif col_idx == 24 and val == "★":
                fill = pref_fill
            align = left if col_idx in (1,2,3,20) else center
            set_cell(ws1, row_idx, col_idx, val, align=align, fill=fill)

    ws1.freeze_panes = "A2"
    ws1.auto_filter.ref = f"A1:X{len(quarterly_scored) + 1}"

    # ---------- 表2：优选股 ----------
    ws2 = wb.create_sheet("年报A∩季报A")
    headers2 = [
        ("代码", 12), ("名称", 12), ("行业", 14),
        ("年报总分", 10), ("年报等级", 8), ("季报总分", 10), ("季报等级", 8),
        ("季报毛利率", 10), ("季报净利率", 10), ("季报营收同比", 12), ("季报利润同比", 12),
        ("年报ROE", 10), ("年报负债率", 10), ("年报现金流", 10),
        ("季报日期", 12), ("置信度", 8),
    ]
    for i, (h, w) in enumerate(headers2, 1):
        set_hdr(ws2, 1, i, h, w)
    ws2.row_dimensions[1].height = 22

    for row_idx, r in enumerate(preferred, 2):
        ann = annual_map.get(r["ts_code"], {})
        row_data = [
            r["ts_code"], r["name"], r.get("industry_l1", ""),
            ann.get("total_score"), ann.get("grade"),
            r.get("quarterly_total_score"), r.get("quarterly_grade"),
            r.get("q_gross_margin"), r.get("q_net_margin"), r.get("q_revenue_yoy"), r.get("q_profit_yoy"),
            r.get("a_roe"), r.get("a_debt_ratio"), r.get("a_ocf_to_profit"),
            r.get("quarterly_report_date", ""), r.get("confidence", ""),
        ]
        for col_idx, val in enumerate(row_data, 1):
            fill = pref_fill if col_idx == 1 else None
            align = left if col_idx in (1,2,3) else center
            set_cell(ws2, row_idx, col_idx, val, align=align, fill=fill)

    ws2.freeze_panes = "A2"
    ws2.auto_filter.ref = f"A1:O{len(preferred) + 1}"
    ...
    stats = [
        ...
        ("股票总数", len(annual_scored)),
        ("年报A级", len([r for r in annual_scored if r.get("grade") == "A"])),
        ("季报A级", len([r for r in quarterly_scored if r.get("quarterly_grade") == "A"])),
        ("交集优选（年报A∩季报A）", len(preferred)),
        ("", ""),
        ("年报评分分布", ""),
        ("  A级", len([r for r in annual_scored if r.get("grade") == "A"])),
        ("  B级", len([r for r in annual_scored if r.get("grade") == "B"])),
        ("  C级", len([r for r in annual_scored if r.get("grade") == "C"])),
        ("  D级", len([r for r in annual_scored if r.get("grade") == "D"])),
        ("  E级", len([r for r in annual_scored if r.get("grade") == "E"])),
        ("", ""),
        ("季报评分分布", ""),
        ("  A级", len([r for r in quarterly_scored if r.get("quarterly_grade") == "A"])),
        ("  B级", len([r for r in quarterly_scored if r.get("quarterly_grade") == "B"])),
        ("  C级", len([r for r in quarterly_scored if r.get("quarterly_grade") == "C"])),
        ("  D级", len([r for r in quarterly_scored if r.get("quarterly_grade") == "D"])),
        ("  E级", len([r for r in quarterly_scored if r.get("quarterly_grade") == "E"])),
    ]
    for row_idx, (label, val) in enumerate(stats, 1):
        c1 = ws3.cell(row=row_idx, column=1, value=label)
        c2 = ws3.cell(row=row_idx, column=2, value=val)
        if label and not str(label).startswith("  "):
            c1.font = Font(bold=True)
            if "A级" in str(label) and "分布" not in str(label):
                c1.fill = PatternFill("solid", fgColor="2F5496")
                c1.font = Font(bold=True, color="FFFFFF")

    wb.save(path)
    logger.info(f"Excel 报告已保存: {path}")

# ========== 主流程 ==========
def main():
    setup_logging()
    parser = argparse.ArgumentParser(description="A股季度评分系统 V6.0.1")
    ...
    parser.add_argument("--local-test", action="store_true", help="本地测试模式")
    ...
    try:
        ...
        logger.error(f"股票列表文件不存在: {Config.DEFAULT_STOCK_FILE}")
        sys.exit(1)
    if not stocks:
        logger.error("股票列表为空")
        sys.exit(1)
    ...
    annual_scored = []
    for s in annual_data:
        annual_scored.append(calc_annual_score(s, industry_groups, all_stocks))

    logger.info("获取季度数据...")
    quarterly_raw = fetch_quarterly_batch(annual_data, token, force_refresh=args.force, conn=conn)
    for r in quarterly_raw:
        save_quarterly(conn, r["ts_code"], r.get("report_date", ""), r.get("metrics", {}), r.get("fetch_success", False))
    ...
    for a in annual_data:
        ...
        if q and q.get("fetch_success"):
            score = calc_quarterly_score(a, q["metrics"], industry_groups, all_stocks, q.get("report_date", ""))
            score["quarterly_total_score"] = score["total_score"]
            score["quarterly_grade"] = score["grade"]
            # 注入年报总分、等级
            score["annual_total_score"] = annual_map[ts_code]["total_score"]
            score["annual_grade"] = annual_map[ts_code]["grade"]
            ...
            s["annual_total_score"] = s["total_score"]
            s["annual_grade"] = s["grade"]
    ...
    json_path = os.path.join(Config.OUTPUT_DIR, f"股票分析数据_{timestamp}.json")
    output = {
        ...
        "preferred_stocks": [
            {"ts_code": r["ts_code"], "name": r["name"], "industry_l1": r["industry_l1"],
             "annual_score": r.get("annual_total_score"), "annual_grade": r.get("annual_grade"),
    ...
    excel_path = os.path.join(Config.OUTPUT_DIR, f"股票业绩评价_{timestamp}.xlsx")
    _write_excel(excel_path, quarterly_scored, annual_scored, preferred)

    if args.verify > 0:
        all_codes = [s["ts_code"] for s in stocks]
        sample_codes = random.sample(all_codes, min(args.verify, len(all_codes)))
        logger.info(f"随机验证 {len(sample_codes)} 只股票...")
        for code in sample_codes:
            a = annual_map.get(code)
