...
def verify_no_fallback():
    ...
    for row in rows:
        ...
        if roe_ttm is None:
            no_roe_count += 1
            print(f"{ts_code}: TTM ROE为空 ❌ (无ROE数据)")
        else:
            excel_roe = excel_row.iloc[3]
            if abs(float(roe_ttm) - float(excel_roe)) < 0.01:
                print(f"{ts_code}: TTM ROE={roe_ttm} ✅")
            else:
                print(f"{ts_code}: TTM ROE={roe_ttm}, Excel={excel_roe} ⚠️")
