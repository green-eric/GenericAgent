...
def create_ima_note(client_id, api_key, title, content):
    ...
    # 根据 IMA API 文档，import_doc 接口需要 content_format 和 content 字段
    # 标题应该包含在 content 中作为 Markdown 标题
    # content_format: 1 表示 Markdown
    ...
    try:
        ...
        # import_doc 返回的是 note_id
        note_id = result.get("data", {}).get("note_id")
        if not note_id:
            # 尝试其他可能的字段名
            note_id = result.get("data", {}).get("doc_id")
            if not note_id:
                raise Exception("创建笔记失败: 未返回 note_id 或 doc_id")
...
def main():
    ...
    try:
        ...
        if not memory_file:
            print("[错误] 没有找到今日记忆文件，跳过同步")
            sys.exit(0)
        
        # 读取内容
        content = read_memory_content(memory_file)
        if not content:
            print("[错误] 记忆文件内容为空，跳过同步")
            sys.exit(0)
        ...
        print(f"[OK] 同步成功！笔记 ID: {note_id}")
        print(f"[日期] 日期: {today}")
        print(f"[内容] 内容长度: {len(content)} 字符")
