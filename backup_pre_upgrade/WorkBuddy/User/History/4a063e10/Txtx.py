    ...
    with _quick_start_lock:
        ...
        try:
            ...
            def _load_news_async() -> None:
                ...
                retry_delay = 5  # 秒（从10秒缩短，加快新闻面板呈现）

                for attempt in range(max_retries + 1):
                    try:
                        log.info(
                            f"快速启动模式：后台获取新闻数据... (尝试 {attempt + 1}/{max_retries + 1})"
                        )
                        # 使用较少的页数加快获取速度（3页，约60条新闻，足够首屏展示）
                        news_items = fetch_all_news(pages=3)
                        formatted_news = format_news_data(news_items)
                        all_news = formatted_news.get("all_news", [])
                        top_news = formatted_news.get("top_news", [])
                        recent_news = formatted_news.get("recent_news", [])

                        # 如果新闻为空且还有重试机会，等待后重试
                        if not all_news and attempt < max_retries:
                            log.warning(
                                f"快速启动模式：获取新闻为空，将在 {retry_delay} 秒后重试"
                            )
                            time.sleep(retry_delay)
                            continue

                        # 创建新的完整数据字典，而不是修改已有的引用
                        # 这确保正在使用旧数据的HTTP线程不会看到部分更新的数据
                        updated_data = dict(quick_data_ref)  # 浅拷贝
                        ...
                        updated_data["news_stats"] = {
                            ...
                            # 修复: after_stock_filter 应该等于 all_news 数量（24小时内有效新闻）
                            "after_stock_filter": len(all_news) if all_news else 0,
                            "avg_score_all": 0.0,
                            "avg_score_picks": 0.0,
                        }

                        # 从新闻计算板块热度，让板块饼图尽早有数据
                        try:
                            ...
                            if sec_sum and isinstance(sec_sum, dict):
                                ...
                                log.info(
                                    f"快速启动模式：板块热度计算完成，{len(sec_sum)}个板块"
                                )
                        except Exception as sec_err:
                            log.warning(f"快速启动模式：板块热度计算失败 {sec_err}")

                        # 更新缓存，后续请求能拿到含新闻和板块的版本
                        # 刷新 TTL
                        server_cache.set(
                            "quick_start_data", updated_data, ttl=QUICK_START_TTL
                        )
                        log.info(
                            f"快速启动模式：更新quick_start_data缓存成功，新闻{len(news_items)}条"
                        )
                        return  # 成功完成
                    except Exception as e:
                        log.warning(
                            f"快速启动模式：后台获取新闻失败 (尝试 {attempt + 1}/{max_retries + 1}): {e}"
                        )
                        if attempt < max_retries:
                            log.info(f"将在 {retry_delay} 秒后重试...")
                            time.sleep(retry_delay)
                        else:
                            log.error("快速启动模式：新闻加载重试次数用尽，不再重试")
