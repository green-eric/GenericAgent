...
def _get_quick_start_data() -> Dict[str, Any]:
    ...
    with _quick_start_lock:
        ...
        try:
            ...
            # 只取前10只股票进行快速行情获取（优化：减少到10只，加快warmup响应）
            quick_symbols = pool_symbols[:10]
            log.info(f"快速启动模式：获取{len(quick_symbols)}只股票的行情数据")

            # 获取行情数据（带超时保护，最多5秒）
            # 优化：从10秒缩短到5秒，避免warmup卡在行情获取上
            ...
            try:
                quotes = future.result(timeout=5)
                log.info(f"快速启动模式：成功获取{len(quotes)}只股票的行情数据")
            except concurrent.futures.TimeoutError:
                log.warning("快速启动模式：获取行情数据超时（5秒），使用空数据")
                quotes = {}
            finally:
