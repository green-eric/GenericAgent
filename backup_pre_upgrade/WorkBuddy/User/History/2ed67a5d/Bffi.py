...
def fetch_industry_by_api(ts_code, name):
    """通过 API 查询单只股票的行业分类，单次超时 30s，避免永久阻塞"""
...
def batch_industry_patch(stocks):
    """行业补调：分批执行，每批之间休眠避免 API 限流，带进度日志"""
    if not Config.RETRY_INDUSTRY_FOR_UNCLASSIFIED or not stocks: return {}
    mapping = {}
    total = len(stocks)
    logger.info(f"行业补调 {total} 只，使用 {Config.INDUSTRY_API_WORKERS} 线程")
    api_failed = []
    done_count = 0
    t_start = time.time()

    # 分批提交，每批最多 50 只，批间休眠 3s 避免触发限流
    BATCH_SIZE = 50
    for batch_start in range(0, total, BATCH_SIZE):
        ...
        with ThreadPoolExecutor(max_workers=Config.INDUSTRY_API_WORKERS) as ex:
            fut = {ex.submit(fetch_industry_by_api, s["ts_code"], s["name"]): s for s in batch}
            for f in as_completed(fut):
                s = fut[f]
                done_count += 1
                try:
                    ind = f.result(timeout=60)  # 单只最多等 60s
                    if ind:
                ...
                if done_count % 20 == 0:
                    ...
                    logger.info(f"行业补调进度: {done_count}/{total} (耗时 {elapsed:.0f}s, 已补调 {len(mapping)} 只)")
