...
def _extract_annual_block(text: str) -> Optional[str]:
    """
    从 NeoData 返回的全文中，精确提取年报段落。
    NeoData 财务复合指标段落的格式为：
...
def _parse_pct_from_line(line: str) -> Optional[float]:
    """从一行文本中提取百分比数值，如 '资产负债率30.18%' -> 30.18"""
    if not line:
        return None
...
def parse_financial_all(content: str) -> dict:
    ...
    if line:
        res["annual_gross_margin"] = _parse_pct_from_line(line)

    # 净利率
    line = _extract_metric_line(block, ["销售净利率"])
    if line:
        res["annual_net_margin"] = _parse_pct_from_line(line)
    ...
    # 净利润（取"净利润"行，不取归母/扣非）
    line = _extract_metric_line(block, ["\n净利润", "净利润"])
    if not line:
        # 尝试带前缀格式
