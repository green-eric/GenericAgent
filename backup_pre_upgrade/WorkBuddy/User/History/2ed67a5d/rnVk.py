...
def run_neodata(query: str) -> dict:
    ...
    if not os.path.isfile(Config.QUERY_SCRIPT):
        logger.error(f"查询脚本缺失: {Config.QUERY_SCRIPT}")
        return {"code":"error","msg":"query_script_not_found"}
    for attempt in range(1, Config.API_RETRY_TIMES+2):
        try:
            ...
            logger.error(f"无法执行查询脚本: {Config.QUERY_SCRIPT}")
            return {"code":"error","msg":"script_not_found"}
...
class FinancialReportParser:
    ...
    def _split(self, text):
        # 【修复】先扫描全文，找出所有"年报"关键词附近的日期，记录为年报日期
        annual_dates = set()
        for m in re.finditer(r'(\d{4}[-/年]?(?:\d{1,2}[-/月]?\d{1,2}[日号]?)?)\s*的?\s*(?:年报|年度报告|全年)', text):
            raw = m.group(1)
            n = self._norm(raw)
            if n:
                annual_dates.add(n)
        for m in re.finditer(r'(\d{4})\s*年?\s*(?:年报|年度报告|全年)', text):
            raw = m.group(1)
            n = self._norm(raw)
            if n:
                annual_dates.add(n)

        # 【修复】用 finditer 找日期位置，避免 re.split 把金额数字误识别为日期
        # 日期格式：YYYYMMDD(8位) 或 YYYY-MM-DD 或 YYYY年MM月DD日 或 YYYY年
        date_pat = re.compile(r'(\d{4})(?:[-/年](\d{1,2})[-/月](\d{1,2})[日号]?|[-/年](\d{1,2})[-/月](\d{1,2})|[-/年]?(\d{1,2})|(?=\D|$))')
        # 更严格的日期匹配：必须是合理的年月日
        strict_date_pat = re.compile(
            r'(?<!\d)'  # 前面不能是数字（避免匹配金额的一部分）
            r'(\d{4})'   # 年
            r'(?:'
            r'[-/年](\d{1,2})[-/月](\d{1,2})[日号]?'  # YYYY-MM-DD / YYYY年MM月DD日
            r'|[-/年](\d{1,2})[-/月](\d{1,2})'        # YYYY-MM / YYYY年MM月
            r'|[-/年](\d{1,2})'                        # YYYY-MM / YYYY年MM
            r'|(?![\d])'                               # 单独的 YYYY（后面不跟数字）
            r')'
        )
        ...
        # 找所有日期位置
        date_positions = []
        for m in strict_date_pat.finditer(text):
            year = int(m.group(1))
            if 2000 <= year <= 2099:  # 合理的年份范围
                date_positions.append((m.start(), m.end(), m.group()))

        # 按日期位置切分文本
        combined = []
        for idx, (start, end, date_str) in enumerate(date_positions):
            ct = text[end:date_positions[idx+1][0]] if idx+1 < len(text) else text[end:]
            ct = text[end:date_positions[idx+1][0]] if idx+1 < len(date_positions) else text[end:]
            n = self._norm(date_str)
            if n:
                seg_type = "unknown"
        ...
        if len(combined) < 1:
            combined = [{"date": None, "content": text, "type": "unknown"}]
        return combined
    ...
    def extract(self, seg):
        ...
        # 【修复】净利润：排除"归母净利润"和"扣非净利润"，只匹配独立的"净利润"
        np_ = re.search(r'(?<!归母)(?<!扣非)净利润\s*([-+]?\d+\.?\d*)\s*(万[亿]?元|亿元|万元|万亿元|千元|元)', c)
        ...
        # 【修复】OCF：排除"每股经营活动产生的现金流量净额"，只匹配总额
        ocf = re.search(r'(?<!每股)经营活动(?:产生|净)?(?:的)?现金流量净额\s*([-+]?\d+\.?\d*)\s*(万[亿]?元|亿元|万元|万亿元|千元|元)', c)
...
def parse_financial_all(content):
    ...
    mapping = [
        ("annual_roe", "roe"), ("annual_gross_margin", "gross_margin"),
    ...
    if a:
        ...
        # 【方案1】没有识别到 annual 段落时，对全部内容直接提取指标
        fake_seg = {"content": content, "date": None}
        ad = parser.extract(fake_seg)
        for t, s in mapping:
            res[t] = ad.get(s)
        # 推断报告日期
        res["annual_report_date"] = _guess_date_from_content(content)
...
def fetch_stock_batch(stocks, workers):
    ...
    total = len(stocks)
    logger.info(f"[批次] {total}只, {workers}线程")
    start_all = time.time()
    executor = ThreadPoolExecutor(max_workers=workers)
    future_to_stock = {executor.submit(fetch_stock_finance, s["ts_code"], s["name"]): s for s in stocks}
    processed_futures = set()
    try:
        for f in as_completed(future_to_stock):
            elapsed = time.time() - start_all
            if Config.GLOBAL_TIMEOUT > 0 and elapsed > Config.GLOBAL_TIMEOUT:
                logger.warning(f"全局超时 {Config.GLOBAL_TIMEOUT}s, 取消剩余任务并记录失败")
            ...
            try:
                ...
                results.append(r)
                # 空数据判断：关键年报指标均为空则视为空
                if all(r.get(k) is None for k in ["annual_roe", "annual_gross_margin", "annual_debt_ratio"]):
                    empty_streak += 1
                    failed += 1
                else:
                    empty_streak = 0
            ...
            if done % 20 == 0:
                elapsed = time.time() - start_all
                logger.info(f"进度: {done}/{total} (耗时 {elapsed:.0f}s, 失败/空: {failed})")
            if empty_streak >= Config.PAUSE_CONSECUTIVE_EMPTY:
                logger.warning(f"暂停{Config.PAUSE_DURATION}s")
    ...
    logger.info(f"批次完成，失败/空: {failed}")
    return results
    ...
    with ThreadPoolExecutor(max_workers=Config.INDUSTRY_API_WORKERS) as ex:
        ...
        for f in as_completed(fut):
            ...
            try:
                ...
                logger.error(f"行业补调异常 {s['ts_code']}: {e}")
                api_failed.append(s)
    for s in api_failed:
...
def run_self_test():
    test_content = """
2024年度报告
营业收入 500.00亿元，同比增长 12.50%；
归属于上市公司股东的净利润 80.00亿元，同比增长 18.20%；
扣除非经常性损益净利润 75.50亿元，同比增长 20.10%；
加权平均净资产收益率ROE为 18.00%；
销售毛利率 45.60%，销售净利率 16.00%；
经营活动产生的现金流量净额 95.00亿元；
总资产周转率 0.85次；
资产负债率 40.50%。
"""
    parser = FinancialReportParser(test_content)
    annual = parser.latest("annual")
    if not annual:
        print("自测失败：未识别到年报段落")
        return
    data = parser.extract(annual)
    print("自测解析结果：")
    for k, v in data.items():
        print(f"  {k}: {v}")
    assert data.get("roe") == 18.0, f"ROE 应为 18.0, 实际 {data.get('roe')}"
    assert data.get("gross_margin") == 45.6
    assert data.get("revenue_yoy") == 12.5
    assert data.get("net_profit") == 80e8
    assert data.get("ocf_abs") == 95e8
    print("自测通过，解析器工作正常。")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="业绩分析系统 V4.0.0",
