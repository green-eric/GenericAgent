...
class Config:
    ...
    MIN_INDICATORS_FOR_SCORING = 4            # 至少需要 4/7 个核心指标才参与评分
...
def parse_financial_all(content: str) -> dict:
    ...
    # 净利润（取"净利润"行，不取归母/扣非/现金含量等衍生指标）
    # 策略：遍历年报段落每一行，找到以"净利润"开头但不含"归母"/"扣非"/"现金含量"的行
    # 注意：必须排除"净利润现金含量"、"净利润增长率"等衍生指标行
    line = None
...
def calc_score(stock, industry_stats, fallback):
    ...
    # 数据完整度门槛：核心指标不足则跳过评分
    if present_count < Config.MIN_INDICATORS_FOR_SCORING:
        return {"total_score": None, "rating": "数据不足",
                "detail": {"score_profit": None, "score_growth": None,
                           "score_ocf": None, "score_debt": None},
                "score_base": {}, "completeness": completeness, "confidence": "低"}
...
def merge_latest_reports(conn, ts_code) -> dict:
    ...
    merged = {
        ...
        "annual_report_date": annual.get("report_date"),
        "fetch_success": True
    }
    return merged
...
def generate_report(results, output_dir, error_list=None, failed_stocks=None):
    ...
    for r in results:
        ...
        # 格式化年报日期：YYYYMMDD -> YYYY-MM-DD
        rd = r.get("annual_report_date")
        if rd and len(rd) == 8:
    ...
    # 分离"数据不足"股票
    df_scored = df[df["评级"] != "数据不足"].copy()
    df_insuff = df[df["评级"] == "数据不足"].copy()

    if not df_scored.empty:
        df_scored["评级"] = df_scored["评级"].astype(pd.CategoricalDtype(categories=["A", "B", "C", "D", "E"], ordered=True))
        df_scored.sort_values(["评级", "总评分"], ascending=[True, False], inplace=True)

    try:
        with pd.ExcelWriter(path, engine='openpyxl') as writer:
            # 综合评价结果：只包含有评级的股票
            if not df_scored.empty:
                df_scored.to_excel(writer, sheet_name='综合评价结果', index=False)
                ws = writer.sheets['综合评价结果']
                col_idx = list(df_scored.columns).index("经营现金流/净利润") + 1
                col_letter = get_column_letter(col_idx)
                for row in range(2, len(df_scored) + 2):
                    cell = ws[f"{col_letter}{row}"]
                    if cell.value is not None:
                        cell.number_format = '0.0000'

                for rate in ["A", "B", "C", "D", "E"]:
                    sub = df_scored[df_scored["评级"] == rate]
                    if not sub.empty:
                        sub.to_excel(writer, sheet_name=f'{rate}级股票', index=False)
                low_conf = df_scored[df_scored["置信度"] == "低"]
            ...
            # 数据不足股票单独一个 sheet
            if not df_insuff.empty:
                df_insuff.to_excel(writer, sheet_name='数据不足', index=False)
            ...
            stats = [
                {"项目": "股票总数", "数值": len(df)},
                {"项目": "有评级股票", "数值": len(df_scored)},
                {"项目": "数据不足", "数值": len(df_insuff)},
                {"项目": "A级", "数值": int((df_scored["评级"] == "A").sum()) if not df_scored.empty else 0},
                {"项目": "B级", "数值": int((df_scored["评级"] == "B").sum()) if not df_scored.empty else 0},
                {"项目": "C级", "数值": int((df_scored["评级"] == "C").sum()) if not df_scored.empty else 0},
                {"项目": "D级", "数值": int((df_scored["评级"] == "D").sum()) if not df_scored.empty else 0},
                {"项目": "E级", "数值": int((df_scored["评级"] == "E").sum()) if not df_scored.empty else 0},
                {"项目": "平均评分", "数值": round(df_scored["总评分"].mean(), 2) if not df_scored.empty else 0},
                {"项目": "最高评分", "数值": round(df_scored["总评分"].max(), 2) if not df_scored.empty else 0},
...
def run_self_test():
    ...
    # 注意：格式与真实 NeoData 返回一致，包含"净利润现金含量"等衍生指标
    test_content = """
...
统计截止日期为20241231的年报，主要财务指标如下（货币单位：人民币元）：
1、资产负债结构方面：
资产负债率40.50%，
产权比率0.11，
速动比率4.78。
...
"""
    ...
    assert data.get("annual_net_profit") == 1642130865.33, f"净利润应为 1642130865.33, 实际 {data.get('annual_net_profit')}"
    ...
    assert abs(data.get("annual_ocf_to_profit") - 95e8/1642130865.33) < 0.01, f"OCF/净利润应为 {95e8/1642130865.33}, 实际 {data.get('annual_ocf_to_profit')}"
...
def main():
    ...
    sorted_r = sorted([r for r in all_results if r.get("total_score") is not None], key=lambda x: x["total_score"], reverse=True)
    output_data = {
        "data_timestamp": datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
        "total_stocks": len(sorted_r),
        "stocks": sorted_r
    }
