...
def fetch_stock_batch(stocks, workers):
    ...
    try:
        for f in as_completed(future_to_stock):
            ...
            if done % 50 == 0:
                ...
                logger.info(f"进度: {done}/{total} ({rate:.1f}/s, 已用{elapsed:.0f}s, 剩余{remaining:.0f}s, 失败:{failed})")
            if empty_streak >= Config.PAUSE_CONSECUTIVE_EMPTY:
                logger.warning(f"暂停{Config.PAUSE_DURATION}s (连续{empty_streak}次空)")
    ...
    logger.info(f"批次完成: {len(results)}/{total}, 失败/空:{failed}, 耗时{elapsed:.0f}s ({rate:.1f}/s)")
    return results
