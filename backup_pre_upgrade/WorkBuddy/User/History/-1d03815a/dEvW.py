...
def verify_stock_fields(excel_file, stock_codes=None):
    ...
    for i, stock_code in enumerate(stock_codes[:10], 1):
        ...
        if not raw_data:
            print(f"   警告: 未找到 {stock_code} 的季度数据")
            continue
        ...
        # 打印验证结果
        print(f"   ROE(TTM): Excel={excel_roe}%, 计算={calc_roe}% {'一致' if excel_roe == calc_roe else '不一致'}")
        print(f"   毛利率(TTM): Excel={excel_gross}%, 计算={calc_gross}% {'一致' if excel_gross == calc_gross else '不一致'}")
        print(f"   净利率(TTM): Excel={excel_net}%, 计算={calc_net}% {'一致' if excel_net == calc_net else '不一致'}")
        print(f"   净利润(TTM): Excel={excel_profit:,.0f}, 计算={calc_profit:,.0f} {'一致' if excel_profit == calc_profit else '不一致'}")
        print(f"   经营现金流(TTM): Excel={excel_ocf:,.0f}, 计算={calc_ocf:,.0f} {'一致' if excel_ocf == calc_ocf else '不一致'}")
    ...
    for result in results:
        ...
        for field, check_result in result['验证项目'].items():
            if isinstance(check_result, dict) and '是否一致' in check_result:
                total_checks += 1
                if check_result['是否一致'] == '一致':
                    passed_verifications += 1
                    status = '通过'
                elif check_result['是否一致'] == '不一致':
                    status = '失败'
                else:
                    status = '无法比较'
