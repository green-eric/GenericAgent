...
| 净利润提取 | 简单关键词匹配 | **正则严格匹配** `^净利润\s*数字\s*单位` + 排除衍生指标 |
| 自测模式 | 无 | **内置 `--test` 模式** |
| 遗留代码 | 含 subprocess/未使用配置项 | **全部清理** |
| 网络层 | 共享 Session | **线程本地 Session**（`thread_local`），每线程独立连接池 |
| 限流计数 | API 错误计数 | **年报块解析失败**计数（`has_valid_block=False`） |
| API 响应 | 仅检查 HTTP 状态 | **额外检查业务状态码** `data.code != "200"` 并重试 |
...
| **并发获取** | 使用线程本地 `requests.Session`（`thread_local`），多线程并发调用 NeoData API |
...
| **报告输出** | 输出 Excel 综合评价报告（8 个 Sheet）、JSON 明细数据 |
...
`run_neodata()` 在 HTTP 成功后还会检查业务状态码 `data.code`，若非 `"200"` 则记录业务错误并重试。重试采用指数退避（`API_RETRY_BACKOFF_BASE=3.0`，即 3s、9s）。
...
**兜底策略**：若未找到年报锚点，`_guess_date_from_trend()` 会尝试从业绩趋势段落提取包含财务关键词（营业收入、净利润、毛利率、ROE 等）的行，作为近似年报段落进行指标提取。若仍无内容，则返回 `fetch_success=False`。
...
> ⚠️ **净利润特殊逻辑**：使用正则 `^净利润\s*数字\s*单位` 严格匹配，确保行首是"净利润"后跟数值和单位，且**不含** `"归母"`、`"扣非"`、`"现金含量"`、`"增长率"`、`"同比"` 等修饰词。若严格匹配失败，再降级为宽泛匹配（包含"净利润"但不含修饰词的行）。
...
- 每项指标在同行业内进行百分位排名，转化为 **0~100** 分值（**排除自身**，避免自己影响自己的排名）
- 行业样本不足 **5** 只时，退化为全市场百分位 × **0.95** 折扣
- ROE 为负值直接给 **0** 分
- 净利润与经营现金流均为负时，总分上限锁定为 **15** 分
- 当池中仅有 1 个参考值时：若当前值 ≥ 参考值得 **100** 分，否则 **0** 分
...
- 每个线程持有独立的 `requests.Session`（`thread_local` + `HTTPAdapter` 连接池，`pool_connections=5, pool_maxsize=5`），避免共享 Session 的线程安全问题
- 行业补调使用独立 `requests.Session` 连接池（`max_retries=1`）

### 8.2 限流保护

- **连续年报块解析失败**达到 **10** 次时（`has_valid_block=False`），暂停 **20 秒**后重置计数器
- API 返回空内容或年报段落提取失败均计入连续解析失败
- 全局超时（`GLOBAL_TIMEOUT` = **7200s**）可通过 `--timeout` 参数调整
...
每处理 **100** 只股票输出一次进度：已处理数 / 总数、当前速率（只/秒）、已用时间 / 预估剩余时间。
...
**评级与元数据（8个）**：`grade`、`confidence`、`completeness`、`completeness_level`、`report_date`、`market_fallback`、`fetch_success`、`ts_code`/`name`/`industry_l1`
