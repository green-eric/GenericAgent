...
def field_verification():
    ...
    for i, row in enumerate(rows):
        ...
        fields_to_check = [
            ("ROE_TTM", row[1], excel_row.iloc[3] if excel_row is not None and len(excel_row) > 3 else None),
            ("净利润_TTM", row[2], excel_row.iloc[10] if excel_row is not None and len(excel_row) > 10 else None),
            ("经营现金流_TTM", row[3], excel_row.iloc[11] if excel_row is not None and len(excel_row) > 11 else None),
            ("毛利率_TTM", row[5], excel_row.iloc[4] if excel_row is not None and len(excel_row) > 4 else None),
            ("净利率_TTM", row[6], excel_row.iloc[5] if excel_row is not None and len(excel_row) > 5 else None),
            ("营收同比(单季)", row[9], excel_row.iloc[6] if excel_row is not None and len(excel_row) > 6 else None),
            ("净利润同比(单季)", row[10], excel_row.iloc[7] if excel_row is not None and len(excel_row) > 7 else None),
            ("资产负债率(单季)", row[11], excel_row.iloc[8] if excel_row is not None and len(excel_row) > 8 else None)
        ]

        for field_name, db_value, excel_value in fields_to_check:
            ...
            if db_value is None:
                ...
                status = "Excel中无数据"
                verification_results[field_name]["failed"] += 1
            elif abs(float(db_value or 0) - float(excel_value or 0)) < 0.01:
                status = "匹配"
