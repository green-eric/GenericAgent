...
class DataProcessor:
    ...
    def calculate_growth_rate(self,
        ...
        if previous_value == 0:
            return 0.0 if current_value is None or pd.isna(current_value) or current_value == 0 else float('inf')

        try:
            # 确保都是数值类型
            current_val = float(current_value) if current_value is not None and not pd.isna(current_value) else 0
