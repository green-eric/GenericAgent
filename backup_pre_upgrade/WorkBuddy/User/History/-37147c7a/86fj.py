...
def main():
    ...
    if args.from_db:
        ...
        for i, symbol in enumerate(symbols, 1):
            ...
            # 回退补取：名称=代码 或 行业为空 或 市值=0 时从行情缓存/API补取
            need_fallback = (
                not name or name == symbol or name == '' or
                not industry or industry == '' or
                quote.get('total_mv', 0) == 0 or quote.get('pe_ttm', 0) == 0
            )
            if need_fallback:
                try:
                    from data_provider import DataProvider, preload_market_data
                    # 首次进入时预加载行情缓存
                    ...
                    if quote.get('total_mv', 0) == 0 or quote.get('pe_ttm', 0) == 0:
