            ...
            if mock:
                ...
                try:
                    ...
                    res = {
                        ...
                        'total_score': scores['total_score'],
                        'rating': get_rating(scores['total_score']),
                        'confidence': get_confidence(scores['total_score'], min(len(df) / 8 * 100, 100)),
                        'profitability': round(scores['profitability'], 2),
                        'growth': round(scores['growth'], 2),
                        'cash_flow_quality': round(scores['cash_flow'], 2),
                        'leverage_risk': round(scores['leverage'], 2),
...
def save_to_excel(results: List[Dict], filename: str = None):
    ...
    order = [
        ...
        'pe_ttm', 'total_mv',
        # 基础信息（日期/完整度，后置）
        'annual_report_date', 'latest_quarter', 'data_completeness',
        # 否决
        'veto', 'veto_reason',
    ]
...
def main():
    parser = argparse.ArgumentParser(description='A股智能选股系统 v3.1')
    ...
    if args.from_db:
        ...
        if results:
            save_to_excel(results, args.output or 'score_result.xlsx')
        return
    ...
    if args.real and db:
        ...
        report_file = os.path.join(log_dir, f'fetch_report_{log_ts}.json')
        
        # 强制 UTF-8 输出（防 Windows 控制台乱码）
        import sys as _sys
        _sys.stdout.reconfigure(encoding='utf-8', errors='replace')
        _sys.stderr.reconfigure(encoding='utf-8', errors='replace')
        
        def log_write(msg, to_console=True):
            """同时写日志文件和打印到控制台（全程UTF-8，绝不乱码）"""
            ...
            if to_console:
                try:
                    ...
                    # 极端情况下控制台不支持UTF-8时，用ASCII安全输出
                    print(line.encode('ascii', errors='replace').decode('ascii'))
            with open(log_file, 'a', encoding='utf-8') as lf:
                lf.write(line + '\n')
        
        log_write(f"{'='*60}")
        log_write(f"全量数据获取启动 | workers={args.workers} rate_limit={args.rate_limit}s")
        ...
        # 断点续传：跳过已有足够数据的股票
        existing = set(db.get_symbols_with_financials(min_quarters=4))
        fetch_symbols = [s for s in symbols if s not in existing]
        skipped = len(symbols) - len(fetch_symbols)
        log_write(f"[断点续传] 总池 {len(symbols)} 只 | 已有数据跳过 {skipped} 只 | 需获取 {len(fetch_symbols)} 只")
        
        if not fetch_symbols:
            log_write("所有股票数据已存在，无需获取")
        else:
            # 统计计数器
            success_count = 0
            fail_count = 0
                ...
                try:
                    ...
                    if df is not None and not df.empty:
                        ...
                        with lock:
                            ...
                            print(f"\r  [{done}/{len(fetch_symbols)}] {pct:.0f}% | ✓{success_count} ✗{fail_count} ∅{empty_count} | {symbol} {name} {elapsed:.1f}s    ", end='', flush=True)
                    else:
                        elapsed = _time.time() - t0
                        with lock:
                            empty_count += 1
                            ...
                            failed_details.append({'symbol': symbol, 'reason': '财务数据为空', 'type': 'empty'})
                            done = success_count + fail_count + empty_count
                            pct = done / len(fetch_symbols) * 100
                            print(f"\r  [{done}/{len(fetch_symbols)}] {pct:.0f}% | ✓{success_count} ✗{fail_count} ∅{empty_count} | {symbol} 数据为空 {elapsed:.1f}s    ", end='', flush=True)
                    ...
                    with lock:
                        ...
                        pct = done / len(fetch_symbols) * 100
                        print(f"\r  [{done}/{len(fetch_symbols)}] {pct:.0f}% | ✓{success_count} ✗{fail_count} ∅{empty_count} | {symbol} [{type(e).__name__}] {elapsed:.1f}s    ", end='', flush=True)
                
                _time.sleep(args.rate_limit)
            
            # ---- 并发执行 ----
            log_write(f"开始并发获取 (workers={args.workers})...")
                ...
                for f in as_completed(futures):
                    try:
                        ...
                        with lock:
                            ...
                            failed_details.append({'symbol': sym, 'error_type': type(e).__name__, 'error_msg': str(e)[:200], 'category': 'fatal'})
            ...
            report = {
                ...
                'total_pool': len(symbols),
                'skipped_existing': skipped,
                'fetched': len(fetch_symbols),
                'success': success_count,
                'failed': fail_count,
                'empty_data': empty_count,
                'error_breakdown': {
                    'network': network_err_count,
                    'timeout': timeout_count,
                    'parse': parse_err_count,
                    'other': other_err_count,
                },
                'elapsed_seconds': round(t_batch_elapsed, 1),
                'avg_seconds_per_stock': round(avg_time, 2),
                'success_rate': f"{success_count/len(fetch_symbols)*100:.1f}%" if fetch_symbols else "N/A",
            ...
            with open(report_file, 'w', encoding='utf-8') as rf:
                _json.dump(report, rf, ensure_ascii=False, indent=2)
            ...
            # 打印汇总
            log_write(f"\n{'='*60}")
            log_write(f"[汇总] 全量数据获取完成")
            log_write(f"{'='*60}")
            log_write(f"  总池: {len(symbols)} 只 | 跳过(已有数据): {skipped} 只 | 实际获取: {len(fetch_symbols)} 只")
            log_write(f"  ✅ 成功: {success_count} 只 ({report['success_rate']})")
            log_write(f"  ❌ 失败: {fail_count} 只")
            log_write(f"    ├─ 数据为空: {empty_count}")
            log_write(f"    ├─ 网络错误: {network_err_count}")
            log_write(f"    ├─ 超时:     {timeout_count}")
            log_write(f"    ├─ 解析错误: {parse_err_count}")
            log_write(f"    └─ 其他错误: {other_err_count}")
            log_write(f"  耗时: {t_batch_elapsed:.0f}s ({t_batch_elapsed/60:.1f}min) | 平均: {avg_time:.1f}s/只")
            log_write(f"  日志: {log_file}")
            log_write(f"  报告: {report_file}")
            if failed_symbols:
                log_write(f"  失败列表: {fail_file} ({len(failed_symbols)} 只)")
            log_write(f"{'='*60}")
        ...
        if args.fetch_only:
            ...
            # 如果有失败股票，提示重跑命令
            if 'failed_symbols' in dir() and failed_symbols:
                log_write(f"\n[提示] 如需重跑失败股票，执行:")
