...
def main():
    parser = argparse.ArgumentParser(description='A股智能选股系统 v3.0')
    ...
    # ========== 模式2：获取数据并存DB（并发+断点续传） ==========
    if args.real and db:
        import time as _time
        t_batch_start = _time.time()
        print(f"\n获取真实数据并存DB (workers={args.workers}, rate_limit={args.rate_limit}s)...")
        
        # 预加载全市场行情缓存（一次拉取，后续O(1)查询）
        from data_provider import preload_market_data
        preload_ok = preload_market_data()
        if preload_ok:
            print("[预加载] 全市场行情缓存加载成功，市值/PE/名称将直接查缓存")
        else:
            print("[预加载] 全市场行情缓存加载失败，将逐只获取（较慢）")
        ...
        if skipped > 0:
            print(f"[断点续传] 跳过 {skipped} 只已有数据股票，需获取 {len(fetch_symbols)} 只")
        
        if not fetch_symbols:
            print("所有股票数据已存在，无需获取")
        else:
            success, fail = 0, 0
            lock = threading.Lock()
            info_batch = []
                ...
                try:
                    ...
                    if df is not None and not df.empty:
                        ...
                        with lock:
                            ...
                            print(f"\r[数据] {success}/{len(fetch_symbols)} 成功, {fail} 失败 | 最新: {symbol} {name} {elapsed:.1f}s", end='', flush=True)
                    else:
                        elapsed = _time.time() - t0
                        with lock:
                            fail += 1
                            print(f"\r[数据] {success}/{len(fetch_symbols)} 成功, {fail} 失败 | {symbol} 财务数据为空 {elapsed:.1f}s", end='', flush=True)
                except Exception as e:
                    elapsed = _time.time() - t0
                    with lock:
                        fail += 1
                        print(f"\n  [FAIL] {symbol}: [{type(e).__name__}: {e}] {elapsed:.1f}s")
                _time.sleep(args.rate_limit)
            
            with ThreadPoolExecutor(max_workers=args.workers) as executor:
                futures = [executor.submit(fetch_one, s) for s in fetch_symbols]
            ...
            print(f"\n数据获取完成: 成功 {success}, 失败 {fail} | 总耗时{t_batch_elapsed:.1f}s, 平均{avg_time:.1f}s/只")
        
        if args.fetch_only:
            stats = db.get_db_stats()
            print(f"[DB统计] 财务记录: {stats['financials']}, 股票数: {stats['symbols_with_fin']}, DB大小: {stats['db_size_mb']}MB")
            return
