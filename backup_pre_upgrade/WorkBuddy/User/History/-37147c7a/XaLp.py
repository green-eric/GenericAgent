...
def main():
    ...
    if args.from_db:
        ...
        for i, symbol in enumerate(symbols, 1):
            ...
            # 从DB读取股票信息（stocks/quotes表可能为空，需回退到行情缓存补取）
            ...
            # 回退补取：名称/行业/行情缺失时从行情缓存+API补取
            need_fallback = (
                not name or name == symbol or name == '' or
                not industry or industry == '' or
                quote.get('total_mv', 0) == 0
            )
            if need_fallback:
                try:
                    from data_provider import DataProvider, preload_market_data, _lookup_spot_cache
                    # 首次进入时预加载行情缓存（一次拉取5000+行，后续O(1)查询）
                    if not hasattr(main, '_market_preloaded'):
                        preload_market_data()
                        main._market_preloaded = True
                    
                    # 优先从行情缓存获取名称/市值/PE（无网络开销）
                    spot = _lookup_spot_cache(symbol)
                    if (not name or name == symbol or name == '') and spot.get('name'):
                        name = spot['name']
                    ...
                    # 名称仍缺则走API
                    if not name or name == symbol or name == '':
                    ...
                    # 补取成功后写回DB，下次无需再取
                    db.save_stock_info(symbol, name, industry)
                    if quote.get('total_mv', 0) > 0:
                        db.save_quote(symbol, quote)
                except Exception as e:
                    logging.debug(f"{symbol} 信息补取失败: {e}")
            ...
            def fetch_one(symbol):
                ...
                try:
                    ...
                    if df is not None and not df.empty:
                        # 增量模式：只保存DB中不存在的新报告期
                        if mode == 'incr':
                            existing_dates = incr_existing_dates.get(symbol, set())
