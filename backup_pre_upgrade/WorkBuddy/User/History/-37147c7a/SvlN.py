                ...
                try:
                    ...
                    if df is not None and not df.empty:
                        ...
                        with lock:
                            ...
                            print(f"\r[数据] {success}/{len(fetch_symbols)} 成功, {fail} 失败", end='', flush=True)
                    else:
                        with lock:
                            fail += 1
                except Exception as e:
    ...
    if args.real and not db:
        ...
        for i, symbol in enumerate(symbols, 1):
            ...
            res = {
                ...
                'pe_ttm': round(quote.get('pe_ttm', 0), 2),
                'total_mv': round(quote.get('total_mv', 0) / 100000000, 2),
                'annual_report_date': str(df[df['report_date'].dt.month == 12]['report_date'].max()) if len(df[df['report_date'].dt.month == 12]) > 0 else str(df['report_date'].max()),
                'latest_quarter': str(df['report_date'].max().date()) if hasattr(df['report_date'].max(), 'date') else str(df['report_date'].max()),
                'data_completeness': min(len(df) / 8 * 100, 100),
                'total_score': scores['total_score'],
                'rating': get_rating(scores['total_score']),
