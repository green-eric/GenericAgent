...
class AStockAnalyzer:
    ...
    def load_stock_list(self) -> bool:
        """加载股票列表（模拟数据）"""
        try:
            # 模拟A股股票列表（实际应用中应从API获取）
            self.stock_list = [
                {"ts_code": f"{code}.SZ", "symbol": symbol, "name": name}
            ...
            self.stock_list = [
                ...
                if not (stock["ts_code"].startswith("688.") or stock["ts_code"].startswith("430."))
            ]

            logger.info(f"加载了 {len(self.stock_list)} 只A股股票")
            return True
