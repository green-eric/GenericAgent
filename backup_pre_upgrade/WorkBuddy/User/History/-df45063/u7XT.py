...
class IndicatorCalculator:
    ...
    def _split_quarterly(self):
        ...
        for col in flow_cols:
            ...
            for qtr, prev_qtr in prev_qtr_map.items():
                curr_mask = df['_qtr'] == qtr
                prev_mask = df['_qtr'] == prev_qtr
                
                # 同一年份的前一季度
                curr_years = df.loc[curr_mask, '_year'].values
                prev_years = df.loc[prev_mask, '_year'].values
                
                for year in set(curr_years):
                    curr_idx = df.index[curr_mask & (df['_year'] == year)]
                    prev_idx = df.index[prev_mask & (df['_year'] == year)]
                    if len(curr_idx) > 0 and len(prev_idx) > 0:
                        curr_val = df.loc[curr_idx[0], col]
                        prev_val = df.loc[prev_idx[0], col]
                        if pd.notna(curr_val) and pd.notna(prev_val):
                            df.loc[curr_idx[0], q_col] = curr_val - prev_val
    ...
    def _calc_yoy(self):
        ...
        # 分别对净利润和营收建日期→index映射（避免有营收无净利润时找不到索引）
        date_idx_profit = {}
        date_idx_revenue = {}
        for i, row in df.iterrows():
            if pd.notna(row.get('q_net_profit_parent')):
                date_idx_profit[row['report_date']] = i
            if pd.notna(row.get('q_revenue')):
                date_idx_revenue[row['report_date']] = i

        col_date_idx = {
            'net_profit_parent': date_idx_profit,
            'revenue': date_idx_revenue,
        }

        for col in ['net_profit_parent', 'revenue']:
            yoy_col = f'q_{col}_yoy'
            df[yoy_col] = np.nan
            d_idx = col_date_idx[col]

            for i, row in df.iterrows():
                q_val = row.get(f'q_{col}')
                if pd.isna(q_val):
                    continue
                ...
                # 查找最接近去年同期的数据
                prev_i = d_idx.get(last_year_date)
                if prev_i is None:
                    # 找最近的
                    closest = None
                    min_diff = pd.Timedelta(days=999)
                    for d, di in d_idx.items():
                        diff = abs(d - last_year_date)
                        if diff < min_diff:
                            min_diff = diff
                            closest = di
                    prev_i = closest
                ...
                if prev_i is not None:
                    base_val = df.at[prev_i, f'q_{col}']
                    if pd.notna(base_val) and base_val != 0:
                        df.at[i, yoy_col] = (q_val / base_val - 1) * 100
