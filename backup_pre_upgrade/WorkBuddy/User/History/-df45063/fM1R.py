...
class IndicatorCalculator:
    ...
    def _split_quarterly(self):
        ...
        # 构建年度索引映射：{(year, qtr): row_index}
        idx_map = {}
        for i, row in df.iterrows():
            key = (row['_year'], row['_qtr'])
            if pd.notna(row['_qtr']):
                idx_map[key] = i
        ...
        for col in flow_cols:
            q_col = f'q_{col}'
            values = df[q_col].values.copy()  # numpy array for fast access
            
            for (year, qtr), idx in idx_map.items():
                if qtr == 1:
                    # Q1: 直接取累计值
                    val = df.at[idx, col]
                    if pd.notna(val):
                        values[idx] = val
                elif qtr in prev_qtr_map:
                    prev_key = (year, prev_qtr_map[qtr])
                    if prev_key in idx_map:
                        prev_idx = idx_map[prev_key]
                        curr_val = df.at[idx, col]
                        prev_val = df.at[prev_idx, col]
                        if pd.notna(curr_val) and pd.notna(prev_val):
                            values[idx] = curr_val - prev_val
