...
class IndicatorCalculator:
    ...
    def _calc_ttm_and_leverage(self):
        ...
        # TTM 流量数据 - 扣非净利润优先
        if 'q_net_profit_ex' in df.columns and not df['q_net_profit_ex'].isna().all():
    ...
    def _calc_profitability(self):
        ...
        self.gross_margin_ttm = ((self._ttm_revenue - self._ttm_oper_cost) / self._ttm_revenue * 100) if self._ttm_revenue else 0
    ...
    def fcf_yield(self) -> float:
        """自由现金流收益率 = (OCF - Capex) / equity_parent"""
        if self._equity_parent:
