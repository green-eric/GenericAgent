...
"""
===============================================================================
  数据获取层 - 基于 AkShare 同花顺接口 V6.0.0
  性能优化 + 详细日志 + 错误可追踪

  优化点（V6.0.0 - 4000+股票优化版）：
  - 三只财务报表并行获取（ThreadPoolExecutor，单只股票内并发）
  ...
  - 财务费用合并：interest_expenses 优先，benefit_finance_fee 兜底（0值保留）
===============================================================================
"""
...
# 注意：logging.basicConfig 不在模块层调用，由 main.py 统一配置 root logger
# 模块层只获取 logger 实例，handler 由 main() 注入
...
_akshare_unavailable: bool = False  # 熔断标志：AkShare API全线不可用
_financial_api_failed: bool = False  # 熔断：财务API全线不可用
_consecutive_failures: int = 0  # 连续失败计数器


def _get_spot_df() -> Optional[pd.DataFrame]:
    """获取全市场行情（带缓存+熔断）"""
    global _spot_cache, _spot_cache_time, _akshare_unavailable
    now = time.time()
    # 熔断：已确认AkShare不可用，直接返回缓存（即使过期）
    ...
    try:
        ...
        # 多次失败后熔断：标记AkShare全线不可用
        if _spot_cache is None:
            _akshare_unavailable = True
            logger.warning("AkShare全线熔断：全市场行情不可用，后续跳过所有spot相关API")
        return _spot_cache
...
def _fetch_stock_info(symbol: str) -> Dict:
    ...
    # 熔断检查：如果AkShare全线不可用，跳过东方财富API
    global _akshare_unavailable
    if _akshare_unavailable and result['industry'] is None:
        result['name'] = spot_data.get('name', symbol) or symbol
...
class DataProvider:
    ...
    def _fetch_and_pivot(cls, symbol, api_func, api_name, needed_metrics):
        ...
        try:
            ...
            elapsed = time.time() - t0
            logger.debug(f"{symbol} {api_name}：失败 [{type(e).__name__}] | {elapsed:.1f}s")
            return None

    @classmethod
    def _fetch_one_statement(cls, symbol, api_pairs, metrics):
        """
        顺序尝试多个API，返回第一个成功的DataFrame。
        【修复】去掉内部并发，防止多股票×多报表×多API导致请求过多被服务器拒绝。
        """
        for api_name, api_func in api_pairs:
            df = cls._fetch_and_pivot(symbol, api_func, api_name, metrics)
            if df is not None and not df.empty:
                return df
        return None
    ...
    def get_combined_financials(cls, symbol: str) -> pd.DataFrame:
        """获取个股完整财务数据，按报告期合并（顺序获取三只报表）"""
        global _consecutive_failures, _financial_api_failed
        t_total = time.time()

        # 三只报表的API对（顺序尝试，第二个作为fallback）
        profit_apis = [
        ...
        # 【修复】顺序获取三只报表，避免并发请求过多
        profit = cls._fetch_one_statement(symbol, profit_apis, PROFIT_METRICS)
        if profit is None or profit.empty:
            elapsed = time.time() - t_total
            logger.error(f"{symbol} 财务数据获取失败：利润表为空 | 总耗时{elapsed:.1f}s")
            _consecutive_failures += 1
            if _consecutive_failures >= DATA_CONFIG.get('max_consecutive_failures', 10):
                _financial_api_failed = True
                logger.warning(f"财务API全线熔断：连续{_consecutive_failures}只失败，后续跳过所有财务API")
            return pd.DataFrame()

        _consecutive_failures = 0  # 重置计数器
        balance = cls._fetch_one_statement(symbol, balance_apis, BALANCE_METRICS)
        if balance is None or balance.empty:
            elapsed = time.time() - t_total
            logger.error(f"{symbol} 财务数据获取失败：资产负债表为空 | 总耗时{elapsed:.1f}s")
            return pd.DataFrame()

        cashflow = cls._fetch_one_statement(symbol, cashflow_apis, CASHFLOW_METRICS)
        ...
        # 字段映射与合并
        # 财务费用优先级合并：interest_expenses > benefit_finance_fee
