...
class DataProvider:
    ...
    def get_combined_financials(cls, symbol: str) -> pd.DataFrame:
        ...
        if 'interest_expenses' in profit.columns and 'benefit_finance_fee' in profit.columns:
            # 优先用interest_expenses，空值/无效值用benefit_finance_fee补
            ie = profit['interest_expenses'].replace('', pd.NA)
            ie = ie.apply(lambda x: pd.NA if (isinstance(x, (int, float)) and (x == 0 or pd.isna(x))) else x)
            bf = profit['benefit_finance_fee'].replace('', pd.NA)
            bf = bf.apply(lambda x: pd.NA if (isinstance(x, (int, float)) and (x == 0 or pd.isna(x))) else x)
            # interest_expenses优先，空值用benefit_finance_fee补
