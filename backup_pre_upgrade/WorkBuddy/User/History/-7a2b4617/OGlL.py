...
def _fetch_stock_info(symbol: str) -> Dict:
    ...
    try:
        ...
        elapsed = time.time() - t0
        logger.warning(f"{symbol} 东方财富个股信息：失败 [{type(e).__name__}] | {elapsed:.1f}s")

    # 备用：NeoData（一次获取名称+行业+市值+PE）
        ...
        try:
            ...
            if nd:
                if not result['name'] or result['name'] == symbol:
                    result['name'] = nd.get('name', symbol)
                if not result['industry']:
                    result['industry'] = nd.get('industry')
...
def _neodata_query(symbol: str) -> Optional[Dict]:
    ...
    返回: {'name': str, 'industry': str, 'total_mv': float(元), 'pe_ttm': float} 或 None
    """
    ...
    try:
        ...
        if nd_result:
            ...
            return nd_result
        logger.warning(f"{symbol} NeoData：未匹配到数据 | {elapsed:.1f}s")
