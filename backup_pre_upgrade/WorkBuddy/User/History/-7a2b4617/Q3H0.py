...
===============================================================================
  ...
  - 模块级缓存：stock_zh_a_spot_em 全市场行情只拉1次（5分钟TTL）
...
def _fetch_stock_info(symbol: str) -> Dict:
    """
    一次性获取股票名称、行业、总市值、PE-TTM（1次API调用代替原来3次）
    """
        ...
        for pe_field in ['市盈率-动态', '市盈率', 'PE(TTM)', 'pe_ttm', '市盈率(动)']:
            ...
            if not pe_rows.empty:
                try:
                    ...
                    continue
        logger.info(f"{symbol} 东方财富个股信息：{result['name']} | {result['industry']} | MV={result['total_mv']:.0f} | PE={result['pe_ttm']:.1f} | {elapsed:.1f}s")
    except Exception as e:
        elapsed = time.time() - t0
        logger.warning(f"{symbol} 东方财富个股信息：失败 [{type(e).__name__}: {e}] | {elapsed:.1f}s")
...
def _neodata_query(symbol: str) -> Optional[Dict]:
    ...
    try:
        ...
        if not os.path.exists(script):
            logger.debug(f"{symbol} NeoData：脚本不存在 {script}")
            return None
        r = subprocess.run(
        ...
        elapsed = time.time() - t0
        if r.returncode != 0:
            logger.warning(f"{symbol} NeoData：返回码={r.returncode} | {elapsed:.1f}s | stderr={r.stderr[:200]}")
            return None
        if not r.stdout.strip():
            logger.warning(f"{symbol} NeoData：输出为空 | {elapsed:.1f}s")
            return None
        data = _json.loads(r.stdout)
        if data.get('code') != '200' or not data.get('suc'):
            logger.warning(f"{symbol} NeoData：code={data.get('code')} suc={data.get('suc')} | {elapsed:.1f}s")
            return None
        ...
        for block in api_data.get('apiRecall', []):
            ...
            if '总市值' in content or '市盈率' in content:
                ...
                if nd_result:
                    logger.info(f"{symbol} NeoData：MV={nd_result.get('total_mv',0):.0f} PE={nd_result.get('pe_ttm',0):.1f} | {elapsed:.1f}s")
                    return nd_result
        logger.warning(f"{symbol} NeoData：未匹配到市值/PE数据 | {elapsed:.1f}s")
    except subprocess.TimeoutExpired:
        elapsed = time.time() - t0
        logger.warning(f"{symbol} NeoData：超时(20s) | {elapsed:.1f}s")
    except _json.JSONDecodeError as e:
        elapsed = time.time() - t0
        logger.warning(f"{symbol} NeoData：JSON解析失败 [{e}] | {elapsed:.1f}s")
    except Exception as e:
        elapsed = time.time() - t0
    ...
    try:
        ...
        if len(lines) >= 3:
            ...
            if wresult:
                ...
                logger.warning(f"{symbol} westock-data：解析无结果 | {elapsed:.1f}s")
            return _westock_cache[symbol]
        else:
            logger.warning(f"{symbol} westock-data：表格行不足({len(lines)}行) | {elapsed:.1f}s | output={output[:200]}")
    except subprocess.TimeoutExpired:
        elapsed = time.time() - t0
...
class DataProvider:
    ...
    def get_stock_quote(symbol: str) -> Dict[str, float]:
        """
        获取实时行情（总市值、PE-TTM）
        降级链：_fetch_stock_info → 全市场行情缓存 → NeoData → 兜底估算
        """
        t0 = time.time()
        ...
        if total_mv <= 0 or pe <= 0:
            try:
                ...
                logger.warning(f"{symbol} 全市场行情备用：失败 [{type(e).__name__}: {e}]")
        ...
        if total_mv <= 0 or pe <= 0:
            try:
                ...
                if nd:
                    if total_mv <= 0 and nd.get('total_mv', 0) > 0: total_mv = nd['total_mv']
                    if pe <= 0 and nd.get('pe_ttm', 0) > 0: pe = nd['pe_ttm']
            except Exception as e:
                logger.warning(f"{symbol} NeoData备用：失败 [{type(e).__name__}: {e}]")
