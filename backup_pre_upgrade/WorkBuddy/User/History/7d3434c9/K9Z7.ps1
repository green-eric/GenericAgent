...
Write-Output ""
Write-Output "=== WorkBuddy Processes (with start time) ==="
Get-Process -Name "WorkBuddy" -ErrorAction SilentlyContinue | Sort-Object WorkingSet64 -Descending | ForEach-Object {
    ...
    $st = $_.StartTime.ToString("HH:mm:ss")
    Write-Output "  PID=$($_.Pid)  Started=$st  Mem=${mem}MB  $type"
}
