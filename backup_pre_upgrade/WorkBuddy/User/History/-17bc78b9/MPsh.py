...
def _merge_news_from_multiple_apis(
    ...
    seen_normalized: List[Tuple[str, set]] = []  # (normalized_title, char_set) 用于快速预筛
        ...
        for item in api_news:
            ...
            if title and len(title) > 4:
                ...
                for seen_norm, seen_chars, seen_len in seen_normalized:
                    ...
                    len_ratio = min(title_len, seen_len) / max(title_len, seen_len) if max(title_len, seen_len) > 0 else 0
                    if len_ratio < 0.6:
                        continue
                    ...
                    # 候选对：进入编辑距离精确计算
                    if _is_duplicate_title(title, _denormalize_cache_key(seen_norm)):
                        is_duplicate = True
                        break
...
def fetch_all_news(
    ...
    if not merge_sources:
        ...
        # 并发获取所有API的新闻
        # all_api_news 已在外层初始化

        # 快速返回策略：当足够多的API返回数据后，不再等待慢API
        min_apis_for_quick_return = 2  # 至少2个API返回即可开始处理
        api_timeout_seconds = 15  # 单个API超时从30秒缩短到15秒
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(future_to_api, timeout=api_timeout_seconds):
                ...
                if (len(all_api_news) >= min_apis_for_quick_return
                        and completed_apis >= total_apis / 2):
                    log.info(
                        f"快速返回: 已收集{len(all_api_news)}个API数据，"
