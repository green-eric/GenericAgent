        ...
        for stock_code in self.stock_list:
            ...
            if not assigned:
                if symbol.startswith(('00', '30')):
                    # 深市股票
                    if any(keyword in symbol for keyword in ['电', '子', '信', '息', '科', '技']):
                        mapping[symbol] = "电子"
                    elif any(keyword in symbol for keyword in ['机', '械', '设', '备', '装']):
                        mapping[symbol] = "机械设备"
                    elif any(keyword in symbol for keyword in ['化', '工', '材', '料']):
                        mapping[symbol] = "化工"
                    else:
                        mapping[symbol] = "其他"
                elif symbol.startswith('60'):
                    # 沪市股票
                    if any(keyword in symbol for keyword in ['电', '能', '网', '气']):
                        mapping[symbol] = "电气设备"
                    elif any(keyword in symbol for keyword in ['药', '疗', '健', '生']):
                        mapping[symbol] = "医药生物"
                    elif any(keyword in symbol for keyword in ['金', '属', '有']):
                        mapping[symbol] = "有色金属"
                    elif any(keyword in symbol for keyword in ['纸', '纺', '织', '服']):
                        mapping[symbol] = "轻工制造"
                    else:
                        mapping[symbol] = "其他"
                else:
