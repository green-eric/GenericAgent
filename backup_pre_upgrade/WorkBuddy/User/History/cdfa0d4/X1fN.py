...
class RealListAnalyzer:
    ...
    def _build_comprehensive_industry_map(self) -> Dict[str, str]:
        ...
        # 为所有可能的股票代码分配行业（包括xuan.txt中未明确列出的）
        all_stock_codes = set()
        for stock_code in self.stock_list:
            symbol = stock_code["symbol"]
            all_stock_codes.add(symbol)

        # 扩展行业规则以包含更多股票
        extended_industry_rules = industry_rules.copy()

        # 按交易所和常见模式添加默认分类
        for symbol in all_stock_codes:
            if symbol not in mapping:  # 如果还没有分类
                if any(code.startswith(symbol[:2]) for codes in extended_industry_rules.values() for code in codes):
                    # 根据前两位代码匹配相似行业
                    prefix = symbol[:2]
                    matched_industry = "其他"
                    for industry, codes in extended_industry_rules.items():
                        if any(code.startswith(prefix) for code in codes):
                            matched_industry = industry
                            break
                    mapping[symbol] = matched_industry
                else:
                    # 根据股票代码特征分类
                    if symbol.startswith('00') or symbol.startswith('30'):
                        mapping[symbol] = "电子" if '科技' in symbol or '信息' in symbol else "机械设备"
                    elif symbol.startswith('60'):
                        if any(keyword in symbol for keyword in ['电', '能', '网']):
                            mapping[symbol] = "电气设备"
                        elif any(keyword in symbol for keyword in ['药', '疗', '健']):
                            mapping[symbol] = "医药生物"
                        else:
                            mapping[symbol] = "电子"
                    else:
