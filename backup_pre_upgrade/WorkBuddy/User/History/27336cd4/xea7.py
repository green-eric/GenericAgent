...
import os, sys, json, time, sqlite3, logging, argparse, re, threading
...
def _guess_date_from_trend(text: str) -> str:
    """兜底策略：在没有年报锚点时，尝试从业绩趋势中提取纯年报相关行（排除明确的季报/半年报）"""
    lines = text.split("\n")
    relevant_lines = []
    in_quarter = False
    for line in lines:
        ...
        if in_quarter:
            continue
        # 收集包含关键财务指标的航
        if any(kw in line_stripped for kw in ["营业收入", "净利润", "毛利率", "ROE", "资产负债率",
                                              "经营活动", "现金流", "归母"]):
...
def parse_financial_all(block: str) -> Dict:
    ...
    # 成长性
    line = _extract_metric_line(block, ["营业收入同比增长", "营收同比增长"])
    if not line:
        line = _extract_metric_line(block, ["营业总收入同比增长"])
    ...
    # 规模与现金流（只使用营业总收入）
    line = _extract_metric_line(block, ["营业总收入"])
    result["revenue"] = _parse_num_from_line(line) if line else None
    # 净利润：严格匹配“净利润 数字”模式（排除包含修饰词的行）
    result["net_profit"] = None
    pattern_np = re.compile(r'^净利润\s*([-+]?\d+\.?\d*)\s*(万亿元|亿元|万元|千元|元)')
    for l in block.split("\n"):
        l = l.strip()
        m = pattern_np.match(l)
        if m:
            # 进一步排除含有“同比”“增长率”“现金含量”等词组
            if not any(x in l for x in ["同比", "增长率", "现金含量", "归母", "扣非"]):
                result["net_profit"] = _parse_num_from_line(l)
                break
    # 如果没有匹配到，尝试宽泛匹配但不含修饰词
    if result["net_profit"] is None:
        for l in block.split("\n"):
            l = l.strip()
            if "净利润" in l and not any(x in l for x in ["同比", "增长率", "现金含量", "归母", "扣非"]):
                result["net_profit"] = _parse_num_from_line(l)
                break
...
CODE_PREFIX_INDUSTRY: Dict[str, str] = {
    "60": "机械设备",    # 600xxx 上海主板：机械、汽车、电力等
    "00": "房地产",      # 000xxx/002xxx 深主板/中小板
    "30": "医药生物",    # 300xxx 创业板
    "68": "电子"         # 688xxx 科创板：电子、计算机等
}
...
def determine_industry(ts_code, name, content, industry_map, use_api=True, session=None) -> Optional[str]:
    ...
    # ② 本地映射表（兼容字符串值和字典值两种格式）
    def _get_industry_l1(key):
    ...
    if ind: return ind
    # ③ 二级行业→一级行业映射（仅字典格式支持）
    for key in (ts_code, code_short):
        val = industry_map.get(key)
        if isinstance(val, dict) and val.get("industry_l2"):
            l2 = val["industry_l2"]
            if l2 in SECONDARY_TO_PRIMARY:
                return SECONDARY_TO_PRIMARY[l2]
    # ④ 名称规则
...
def fetch_industry_by_api(ts_code, name, session=None) -> Optional[str]:
    ...
    # 复用 run_neodata 的重试机制
    text = run_neodata(query, token)
    if not text:
        return None
    # 尝试从返回内容中提取行业
    ind = extract_industry_from_content(text)
    return ind
    ...
    for s in stocks:
        ...
        if row:
            ...
            item.update({
                ...
                "report_date": report.get("report_date"),  # 统一用 report_date
            })
        else:
    ...
    for attempt in range(1, Config.API_RETRY_TIMES + 2):
        try:
            ...
            if data.get("code") != "200":
                logger.warning(f"业务错误 (attempt {attempt}): {data.get('msg', '未知')} query: {query[:50]}")
...
def fetch_stock_finance(ts_code, name, token, year=None) -> Dict:
    """获取单只股票财务数据，使用线程本地 session"""
    query = f"{ts_code} {name} 年报"
    try:
        text = run_neodata(query, token)
        if not text:
            return {"metrics": {}, "content": "", "report_date": "", "fetch_success": False, "has_valid_block": False}
        ...
        if not block:
            ...
            has_valid = bool(block)
        if not block:
            return {"metrics": {}, "content": text, "report_date": "", "fetch_success": False, "has_valid_block": False}
        metrics = parse_financial_all(block)
        m = re.search(r"统计截止日期为(\d{4})1231的年报", text)
        report_date = m.group(1) + "1231" if m else ""
        return {"metrics": metrics, "content": text, "report_date": report_date, "fetch_success": True, "has_valid_block": has_valid}
    except Exception as e:
...
def fetch_stock_batch(stocks, token, workers=Config.FINANCE_WORKERS, force_refresh=False, conn=None) -> List[Dict]:
    """批量获取财务数据，线程安全 Session"""
    total = len(stocks)
    results = []
    consecutive_parse_failures = 0  # 基于年报块解析失败计数
    start_time = time.time()
    logger.info(f"开始获取财务数据: 共 {total} 只, {workers} 线程")
    with ThreadPoolExecutor(max_workers=workers) as executor:
        futures = {}
        for s in stocks:
            if not force_refresh and conn and not should_refresh(conn, s["ts_code"], datetime.now().year):
                continue
            future = executor.submit(fetch_stock_finance, s["ts_code"], s["name"], token)
            futures[future] = s
        done_count = 0
        for future in as_completed(futures):
            ...
            try:
                ...
                if consecutive_parse_failures >= Config.PAUSE_CONSECUTIVE_EMPTY:
                    logger.warning(f"连续 {consecutive_parse_failures} 次解析失败，暂停 {Config.PAUSE_DURATION}s")
                    time.sleep(Config.PAUSE_DURATION)
                    consecutive_parse_failures = 0
            except Exception as e:
                consecutive_parse_failures += 1
                results.append({**s, "metrics": {}, "content": "", "report_date": "", "fetch_success": False, "has_valid_block": False})
                logger.error(f"任务异常 {s['ts_code']}: {e}")
            if done_count % 100 == 0 and done_count > 0:
                elapsed = time.time() - start_time
                rate = done_count / elapsed if elapsed > 0 else 0
                eta = (total - done_count) / rate if rate > 0 else 0
                logger.info(f"进度: {done_count}/{total} ({done_count/total*100:.1f}%), 速率:{rate:.1f}/s, 已用:{elapsed:.0f}s, 剩余:{eta:.0f}s")
    elapsed = time.time() - start_time
    logger.info(f"获取完成: {len(results)}/{total}, 耗时:{elapsed:.0f}s")
    return results
...
def calc_completeness(metrics):
    ...
    if ratio >= 0.857: return ratio, "high"
    elif ratio >= 0.571: return ratio, "medium"
...
def percentile_rank(value, values, reverse=False) -> float:
    ...
    if reverse:
        # 对于反向指标（值越小越好），得分 = (有多少比例的 pool >= value) 
        # 即：1 - (有多少比例的 pool <= value)
        score = ((n - count_leq) / n) * 100
    else:
        # 正向指标：得分 = (有多少比例的 pool <= value)
        score = (count_leq / n) * 100
...
def output_excel(results: List[Dict], output_dir=Config.OUTPUT_DIR):
    ...
    for r in sorted_results:
        if not r.get("fetch_success"):
            ws_fail.append([r.get("ts_code",""), r.get("name",""), "API 未返回有效年报数据"])
...
def main():
    ...
    if not args.no_industry_patch:
        # 对未获取到行业的股票，尝试 API 补全
        thread_session = get_thread_session()
        for s in all_stocks:
            if not s.get("industry_l1"):
                ind = determine_industry(s["ts_code"], s["name"], "", industry_map, use_api=True, session=thread_session)
                if ind:
                    s["industry_l1"] = ind
