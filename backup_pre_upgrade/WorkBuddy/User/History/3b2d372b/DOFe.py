...
from annual_scorer import calc_completeness, percentile_rank, Config
...
# 获取宏和科技的最新年报
row = conn.execute(
    "SELECT * FROM financial_reports WHERE ts_code = ? AND fetch_success = 1 ORDER BY report_date DESC LIMIT 1",
    (TARGET,)
).fetchone()
...
# 构建 metrics dict（与 calc_score 收到的格式一致）
metrics = {
    "roe": row["roe"],
...
# 获取名称
name_row = conn.execute("SELECT name FROM stocks WHERE ts_code = ?", (TARGET,)).fetchone()
metrics["name"] = name_row["name"] if name_row else ""
metrics["fetch_success"] = True
metrics["report_date"] = row["report_date"]

print("=" * 60)
print("【宏和科技 603256.SH】完整评分计算过程")
print("数据来源: 缓存数据库 | 年报日期:", row["report_date"])
print("=" * 60)
...
# 获取行业
ind_row = conn.execute("SELECT industry_l1 FROM stocks WHERE ts_code = ?", (TARGET,)).fetchone()
...
if industry:
    peer_rows = conn.execute("""
        ...
        WHERE s.industry_l1 = ? AND f.fetch_success = 1 AND f.ts_code != ?
        GROUP BY f.ts_code
        HAVING f.report_date = MAX(f.report_date)
...
for pr in peer_rows:
    m = {
        "roe": pr["roe"], "gross_margin": pr["gross_margin"],
        "net_margin": pr["net_margin"], "revenue_yoy": pr["revenue_yoy"],
        "profit_yoy": pr["profit_yoy"], "debt_ratio": pr["debt_ratio"],
        "net_profit": pr["net_profit"], "ocf_abs": pr["ocf_abs"],
        "ts_code": pr["ts_code"]
    }
    pool.append(m)
...
if use_market_fallback:
    print(f"  同行业{len(pool)}只 < {Config.MIN_INDUSTRY_SAMPLES} -> 全市场池 x {Config.MARKET_FALLBACK_DISCOUNT}")
    all_rows = conn.execute("""
        SELECT * FROM financial_reports
        WHERE fetch_success = 1 AND ts_code != ?
        GROUP BY ts_code
    ...
    print(f"  同行业{len(pool)}只 >= {Config.MIN_INDUSTRY_SAMPLES} -> 同行业池 x 1.0")
