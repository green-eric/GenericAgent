...
def _guess_date_from_trend(text: str) -> str:
    """兜底策略：当未找到年报锚点时，尝试从业绩趋势段落提取内容。
    查找包含财务关键词的段落作为年报内容的近似替代。"""
    # 尝试找包含"营业收入"或"净利润"的连续段落
    lines = text.split("\n")
    relevant_lines = []
    for line in lines:
        line = line.strip()
        if not line:
            continue
        # 跳过明确的季报/半年报锚点
        if re.search(r"统计截止日期为\d{4}(0331|0630|0930)的", line):
            continue
        # 收集包含财务关键词的行及其上下文
        if any(kw in line for kw in ["营业收入", "净利润", "毛利率", "ROE", "资产负债率",
                                      "经营活动", "现金流", "归母"]):
...
def parse_financial_all(block: str) -> Dict:
    ...
    if not line:
        line = _extract_metric_line(block, ["营业收入"])
...
def determine_industry(ts_code, name, content, industry_map, use_api=True, session=None) -> Optional[str]:
    ...
    # ③ 二级行业→一级行业映射
    if ts_code in industry_map and industry_map[ts_code].get("industry_l2"):
        l2 = industry_map[ts_code]["industry_l2"]
        if l2 in SECONDARY_TO_PRIMARY:
            return SECONDARY_TO_PRIMARY[l2]
    if code_short in industry_map and industry_map[code_short].get("industry_l2"):
        l2 = industry_map[code_short]["industry_l2"]
        if l2 in SECONDARY_TO_PRIMARY:
            return SECONDARY_TO_PRIMARY[l2]
    # ④ 名称规则
...
def calc_completeness(metrics):
    ...
    if ratio >= 0.857: return ratio, "high"   # 6/7 or more
    elif ratio >= 0.571: return ratio, "medium"  # 4/7
...
def output_excel(results: List[Dict], output_dir=Config.OUTPUT_DIR):
    ...
    # Sheet 7: 低置信度股票（完整度为低或极低）
    ws_low = wb.create_sheet("低置信度股票")
    for col, h in enumerate(headers, 1):
        cell = ws_low.cell(row=1, column=col, value=h)
        cell.font = header_font
        cell.fill = header_fill
    for r in sorted_results:
