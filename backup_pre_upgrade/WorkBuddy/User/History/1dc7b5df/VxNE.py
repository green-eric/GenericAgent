...
check("带大数", parse_num("12500000000.00") == 12500000000.0)
...
block3, date3 = _extract_quarterly_block("\u7edf\u8ba1\u622a\u6b22\u65e5\u671f\u4e3a20251231\u7684\u5e74\u62a5")
check("只有年报时返回空", block3 == "" and date3 == "")

block4, date4 = _extract_quarterly_block("\u7edf\u8ba1\u622a\u6b22\u65e5\u671f\u4e3a20250930\u7684Q3\u5355\u5b63\u62a5\n\u9500\u552e\u6bdb\u5229\u738740%")
...
check("升序-最大值=100", percentile_rank(100, [10, 20, 30, 100]) == 100.0)
check("升序-最小值=0", percentile_rank(10, [10, 20, 30, 100]) == 0.0)
check("升序-中间值=50", percentile_rank(20, [10, 20, 30]) == 50.0)
...
stocks = [
    {"ts_code": "000001.SZ", "name": "T1", "industry_l1": "银行",
    ...
    {"ts_code": "000002.SZ", "name": "T2", "industry_l1": "银行",
     ...
     "fetch_success": True, "annual_report_date": "20251231"},
    {"ts_code": "000003.SZ", "name": "T3", "industry_l1": "银行",
...
neg_stock = {
    "ts_code": "000004.SZ", "name": "Neg", "industry_l1": "银行",
    "roe": -5.0, "gross_margin": 30.0, "net_margin": 10.0,
    "revenue_yoy": -5.0, "profit_yoy": -10.0, "debt_ratio": 60.0,
    "ocf_to_profit": 80.0, "net_profit": -1e8, "ocf_abs": -1e8,
    "fetch_success": True, "annual_report_date": "20251231"
...
check("负ROE的profit_score>=0(其他指标有分)", s_neg["profit_score"] >= 0.0)
