...
def check_database():
    ...
    if count > 0:
        cur = conn.execute('SELECT ts_code, name, roe, gross_margin, net_margin, revenue_yoy, profit_yoy FROM financial_reports WHERE report_type="annual" AND fetch_success=1 LIMIT 5')
        rows = cur.fetchall()
        print('\n前5条年报数据:')
        for row in rows:
